                ...
                @foreach ($portfolios as $portfolio)
                    ...
                    <x-modal name="edit-portfolio-{{ $portfolio->id }}">
                        <form action="{{ route('portfolio.update', $portfolio) }}" method="POST"
                            ...
                            onsubmit="var btn = this.querySelector('button[type=submit]'); if (btn) { if (btn.getAttribute('data-submitting') === 'true') { return false; } btn.setAttribute('data-submitting', 'true'); btn.disabled = true; btn.innerHTML = '<i class=\'fa-solid fa-spinner fa-spin me-2\'></i> Menyimpan...'; btn.style.opacity = '0.6'; }">
                            @csrf
                            @method('PUT')
